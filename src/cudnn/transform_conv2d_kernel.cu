...
void TransformConv2D::map(void)
{
  // create descriptors
  checkCUDNN(cudnnCreateTensorDescriptor(&inputTensor));
  checkCUDNN(cudnnCreateTensorDescriptor(&inputTransformedTensor));
  ...
  int groups = inputC / inputs[1].dim[1];
  int padH, padW;
  get_padding(&padH, &padW);
  // set descriptors
  if (needTransform)
  {
      checkCUDNN(cudnnSetTensor4dDescriptor(inputTransformedTensor, dst_layout,
        CUDNN_DATA_FLOAT, inputN, inputC, inputH, inputW));
  }
  checkCUDNN(cudnnSetTensor4dDescriptor(inputTensor, src_layout,
      CUDNN_DATA_FLOAT, inputN, inputC, inputH, inputW));
  ...
  checkCUDNN(cudnnSetTensor4dDescriptor(outputTensor, dst_layout,
  ...
  // allocate tensors
  size_t inputSize = sizeof(DATATYPE) * inputN * inputH * inputW * inputC;
  ...
  // allocate space for transformed input
  if (needTransform)
    checkCUDA(cudaMalloc(&outputs[1].data_ptr, inputSize));
}

void TransformConv2D::unmap(void)
{
  checkCUDNN(cudnnDestroyTensorDescriptor(inputTensor));
  checkCUDNN(cudnnDestroyTensorDescriptor(inputTransformedTensor));
...
void TransformConv2D::forward(bool block)
{
  const float alpha = 1.0f;
  const float beta = 0.0f;
  cudnnTensorDescriptor_t trueConv2dInputTensor = inputTensor;
  void* trueConv2dInputPtr = inputs[0].data_ptr;
  if (needTransform)
  {
    checkCUDNN(cudnnTransformTensor(
        model->dnn, &alpha, inputTensor, inputs[0].data_ptr,
        &beta, inputTransformedTensor, outputs[1].data_ptr));
    trueConv2dInputTensor = inputTransformedTensor;
    trueConv2dInputPtr =  outputs[1].data_ptr;
  }
  
  if (activation != AC_MODE_NONE) {
    checkCUDNN(cudnnConvolutionBiasActivationForward(
        model->dnn, &alpha, trueConv2dInputTensor, trueConv2dInputPtr, filterDesc, inputs[1].data_ptr,
...
void Model::measure_transform_conv2d_cost(TransformConv2D* conv)
{
  const float alpha = 1.0f;
  const float beta = 0.0f;
  ...
  if (conv->needTransform)
  {
      checkCUDNN(cudnnSetTensor4dDescriptor(inputTransformedTensor, conv->dst_layout,
        CUDNN_DATA_FLOAT, inputN, inputC, inputH, inputW));
  }
  checkCUDNN(cudnnSetTensor4dDescriptor(inputTensor, conv->src_layout,
      CUDNN_DATA_FLOAT, inputN, inputC, inputH, inputW));
  checkCUDNN(cudnnSetTensor4dDescriptor(biasTensor, CUDNN_TENSOR_NCHW,
      CUDNN_DATA_FLOAT, 1, outputC, 1, 1));
  checkCUDNN(cudnnSetFilter4dDescriptor(filterDesc, CUDNN_DATA_FLOAT,
  ...
  assert(outputH == h);
  assert(outputW == w);
  checkCUDNN(cudnnSetTensor4dDescriptor(outputTensor, conv->dst_layout,
      CUDNN_DATA_FLOAT, n, c, h, w));
  ...
  cudnnTensorDescriptor_t trueConv2dInputTensor = inputTensor;
  void* trueConv2dInputPtr = inputPtr;
  if (conv->needTransform)
  {
    checkCUDNN(cudnnTransformTensor(
        dnn, &alpha, inputTensor, inputPtr,
        &beta, inputTransformedTensor, inputTransformedPtr));
    trueConv2dInputTensor = inputTransformedTensor;
    trueConv2dInputPtr = inputTransformedPtr;
  }
  ...
  checkCUDNN(cudnnFindConvolutionForwardAlgorithmEx(
      dnn, trueConv2dInputTensor, trueConv2dInputPtr, filterDesc, filterPtr, convDesc,
  ...
  for (int i = 0; i < WARMUP_TIMES + REPEAT_TIMES; i++) {
    ...
    if (conv->activation != AC_MODE_NONE) {
      checkCUDNN(cudnnConvolutionBiasActivationForward(
          dnn, &alpha, trueConv2dInputTensor, trueConv2dInputPtr, filterDesc, filterPtr,
  ...
  if (print_cost)
    printf("  measure[TransformConv2D]: i(%d %d %d %d) w(%d %d %d %d) s(%d %d) p(%d %d) layout(%d->%d) cost(%.4lf)\n",
           ...
           conv->strideH, conv->strideW, padH, padW, conv->src_layout, conv->dst_layout, conv->runtime);
}
